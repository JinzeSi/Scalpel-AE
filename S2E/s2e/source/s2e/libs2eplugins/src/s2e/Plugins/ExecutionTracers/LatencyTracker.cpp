...
void LatencyTracker::onTranslateSpecialInstructionEnd(ExecutionSignal *signal, S2EExecutionState *state,
    ...
    enum special_instruction_t type, const special_instruction_data_t *inst) {
...
void LatencyTracker::onTranslateInstruction(ExecutionSignal *signal,
  ...
  if (is_profileAll) {
    ...
    functionMonitor = s2e()->getPlugin<FunctionMonitor2>();
    if (!functionMonitor) {
...
void LatencyTracker::functionCallMonitor(S2EExecutionState *state, FunctionMonitor2State *fms) {
  DECLARE_PLUGINSTATE(LatencyTrackerState, state);
