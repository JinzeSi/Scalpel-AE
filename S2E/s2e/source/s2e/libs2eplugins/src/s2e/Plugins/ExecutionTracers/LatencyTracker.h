...
class LatencyTracker : public Plugin, public IPluginInvoker {
  ...
  private:
    ...
    FunctionMonitor2* functionMonitor;
    LinuxMonitor* linuxMonitor;
    FunctionMonitor2::CallSignal* callSignal;
  ...
  public:
    ...
    void functionCallMonitor(S2EExecutionState* state, FunctionMonitor2State* fms);
    ...
    void onTranslateSpecialInstructionEnd(ExecutionSignal *signal, S2EExecutionState *state,TranslationBlock *tb, uint64_t pc, enum special_instruction_t type, const special_instruction_data_t *inst);
