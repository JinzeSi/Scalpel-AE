...
if [[ -n "${MYSQL_ANALYSIS_DIR:-}" ]]; then
    MYSQL_ANALYSIS_DIR=$(realpath "$MYSQL_ANALYSIS_DIR") || exit 1
    [[ "$(basename "$MYSQL_ANALYSIS_DIR")" == "$2" ]] || {
        echo "Analysis directory must belong to commit $2." >&2; exit 1;
    }
    [[ "$(git -C "$MYSQL_RUN_ROOT/mysql-server" rev-parse "$2^1")" == "$1" ]] || {
        echo "Replay requires the actual first parent of $2." >&2; exit 1;
    }
    [[ -z "$(git -C "$MYSQL_RUN_ROOT/mysql-server" status --porcelain --untracked-files=no)" ]] || {
        echo "Replay requires a clean mysql-server worktree." >&2; exit 1;
    }
    for analysis_file in getFuncName-ini.txt new_function.txt getFuncName.txt call_analyse.txt symbolic_analyse.txt call_analyse2.txt; do
        [[ -r "$MYSQL_ANALYSIS_DIR/$analysis_file" ]] || {
            echo "Missing archived analysis: $MYSQL_ANALYSIS_DIR/$analysis_file" >&2; exit 1;
        }
    done
    [[ -s "$MYSQL_ANALYSIS_DIR/call_analyse2.txt" ]] || exit 1
    for result_dir in "$MYSQL_RUN_ROOT/$2" "$MYSQL_RUN_ROOT/$1-old"; do
        [[ ! -e "$result_dir" ]] || {
            echo "Result already exists; preserve it before replay: $result_dir" >&2; exit 1;
        }
    done
else
    require_peer_run || exit 1
    peer_is_ready || { echo "Live Pre-knowledge is not ready." >&2; exit 1; }
fi
...
if [ ! -s "getFuncName-ini.txt" ]; then
    ...
    # Keep both workers on the same commit even when analysis finds no code changes.
    rm call_analyse.txt
    rm symbolic_analyse.txt
    touch call_analyse.txt
    touch symbolic_analyse.txt
    request_preknowledge || exit 1
    exit 0
fi
fi
...
rm symbolic_analyse.txt
cat call_analyse.txt | ./symbolic_analyzer++ ./build-ae-new-tmp/compile_commands.json
...
echo "$command_now: Get testcase done."
    ...
    echo "$command_now: Reuse archived Pre-knowledge reply: $MYSQL_ANALYSIS_DIR"
    cd "$MYSQL_RUN_ROOT/mysql-server" || exit 1
    git checkout "$command_now" || exit 1
    for analysis_file in getFuncName-ini.txt new_function.txt getFuncName.txt call_analyse.txt symbolic_analyse.txt call_analyse2.txt; do
        cp -- "$MYSQL_ANALYSIS_DIR/$analysis_file" "$analysis_file"
        relocate_analysis_paths "$analysis_file"
    done
    cp build-ae-new-tmp/compile_commands.json "../$command_now/compile_commands.json.new"
    cp build-ae-old-tmp/compile_commands.json "../$command_now/compile_commands.json.old"
    printf 'archive=%s\nparent=%s\ncommit=%s\n' "$MYSQL_ANALYSIS_DIR" "$command" "$command_now" > "../$command_now/replay-input.txt"
    sha256sum "$MYSQL_ANALYSIS_DIR/call_analyse2.txt" >> "../$command_now/replay-input.txt"
fi
...
for block in "${blocks[@]}"; do
    ...
    if [ ! -f "$file_path_mysql_server" ]; then
        ...
        timeout_time=600
        # 执行脚本并设置超时
        timeout --kill-after=10s "$timeout_time" ./launch-s2e.sh
        s2e_status=$?
        # 检查命令的退出状态
        if [ "$s2e_status" -eq 124 ] || [ "$s2e_status" -eq 137 ]; then
    ...
    if [ ! -f "$file_path_mysql_server" ]; then
        ...
        for src_file in $src_files; do
            src_file_basename=$(basename "$src_file")
            # clang-format -i "$src_file"
            cp "${src_file}" "../${command}-old/$i/${src_file_basename}.real.old"
        done
        cp "sql/sql_parse.cc" "../$command_now/$i/sql_parse.cc.real.old-1"
