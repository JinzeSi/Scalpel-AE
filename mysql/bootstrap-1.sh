#!/bin/bash
#
# This file was automatically generated by s2e-env at 2025-03-03 06:38:20.585026
#
...
function execute_target {
    ...
    # mysqld is dynamically linked, so s2e.so has been preloaded to
...
${S2ECMD} get "mysqld"
${S2ECMD} get "mysql"
${S2ECMD} get "tables.tar.gz"
${S2ECMD} get "output.tar.gz"
...
prepare_target './mysql'

tar -xzvf tables.tar.gz -C .
tar -zxvf output.tar.gz -C .
chmod +x library_output_directory/*

# LD_LIBRARY_PATH=$(pwd) ./mysqld --initialize --user=s2e --basedir=. --datadir=./data
LD_LIBRARY_PATH=$(pwd)/library_output_directory ./mysqld --basedir=$(pwd) --datadir=$(pwd)/data --socket=$(pwd)/mysql.sock --log-error=$(pwd)/error.log --thread_stack=2097152 &
# echo "Waiting for mysqld to be ready..."

# Wait for the server to start, until ready for connections
while ! grep -q "ready for connections" $(pwd)/error.log; do
    echo "Waiting for mysqld to be ready..."
    sleep 10
done

echo "mysqld is ready. Proceeding to start mysql."

./mysql -u root -S $(pwd)/mysql.sock -D test -e "UPDATE comments SET post_id=post_id+1 WHERE id>1000;" 

sleep 5

${S2ECMD} put "error.log"