...
mysql_connection::mysql_connection(const std::string &conninfo)
{
  ...
  char host[100] = {}, port[10] = {}, socket[100] = {};
  char db[100] = {}, user[100] = {}, password[100] = {};

  sscanf(conninfo.c_str(), "host=%s port=%s socket=%s db=%s user=%s password=%s",
          host, port, socket, db, user, password);

  con = mysql_init(NULL);
  if (!mysql_real_connect(con, host, user, password, 
			  db /*dbname*/, 0, socket, 0)) {
  ...
  : mysql_connection(conninfo)
{
...

  {
	...
	throw std::runtime_error("broken schema");
      ...
      string table_name = string(row[1]);
      if (table_name == "performance_schema" || table_name == "sys" || table_name == "mysql" || table_name == "information_schema")
        continue;
