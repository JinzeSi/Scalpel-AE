...
if os.environ.get("APOLLO_TMP_DIR"):
    CONF["OUTDIR"] = os.environ["APOLLO_TMP_DIR"]

if os.environ.get("APOLLO_MINIMIZER_DIR"):
    CONF["MINIMIZER_DIR"] = os.environ["APOLLO_MINIMIZER_DIR"]

if os.environ.get("APOLLO_MYSQL_QUERY_TIMEOUT_MS"):
    mysql_timeout_ms = os.environ["APOLLO_MYSQL_QUERY_TIMEOUT_MS"]
    try:
        mysql_timeout_ms = int(mysql_timeout_ms)
    except ValueError:
        raise RuntimeError("APOLLO_MYSQL_QUERY_TIMEOUT_MS must be an integer")
    if mysql_timeout_ms <= 0:
        raise RuntimeError("APOLLO_MYSQL_QUERY_TIMEOUT_MS must be positive")

    init_option = (
        "mysql --init-command='SET SESSION max_execution_time=%d' " %
        mysql_timeout_ms)
    for command_name in ("RUN_OLD", "RUN_NEW"):
        CONF[command_name] = CONF[command_name].replace(
            "mysql ", init_option, 1)
...
# Python 2 dictionaries do not guarantee iteration order. Timing lists must
# always be old first and new second because the regression ratio depends on it.
if OLDEST_VERSION_PORT == LATEST_VERSION_PORT:
    SERVER_PORTS = [str(OLDEST_VERSION_PORT)]
else:
    SERVER_PORTS = [str(OLDEST_VERSION_PORT), str(LATEST_VERSION_PORT)]
...
class Logger (object):
    def __init__(self, target_db):
        if target_db == "postgres":
            ...
            self.logfile = "fuzz_log_my"
        elif target_db == "sqlite":
            self.logfile = "fuzz_log_sq"
        elif target_db == "cockroach":
            self.logfile = "fuzz_log_cr"

        if os.environ.get("APOLLO_FUZZ_LOG"):
            self.logfile = os.environ["APOLLO_FUZZ_LOG"]
