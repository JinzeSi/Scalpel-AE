...
def retrieve_actual_time_for_mysql(query_out):
    # use small initial number to avoid div by zero
    e_time = MIN
    for line in query_out.split("\n"):
        if "EXPLAIN ANALYZE" in line:
            parts = line.split()
            if len(parts) >= 2:
                e_time = float(parts[1].strip())

    return float(e_time)

def _run_cmd(cmd, ret_elapsed=True):
...
def check_daemon_for_mysql():
    check_cmd = "netstat -napt 2>/dev/null |grep LIST \
        |grep \"%d\\|%d\"|wc -l" \
        % (LATEST_VERSION_PORT, OLDEST_VERSION_PORT)
    output = commands.getoutput(check_cmd)
    output = output.strip()
...
def extract_valid_query(outdir, targetdb):
    ...
    with open(TMP_ERR_PN, 'r') as f:
        ...
        results = ""
        if targetdb == 'mysql':
            if "type found: varchar" in data and "quer" in data:
                results = data.split("type found: varchar")[1].split("queries:")[0]
...
class RegressionFuzzer(object):
    def __init__(
        ...
        self.result_dir = os.environ.get("APOLLO_RESULT_DIR")
        if self.targetdb == 'mysql' and not self.result_dir:
            raise RuntimeError(
                "APOLLO_RESULT_DIR must name a dedicated result directory")
        if not self.result_dir:
            self.result_dir = "/home/sjz/apollo/data"

        if self.targetdb == 'postgres':
            self.dsn = CONF["DSN"]

        elif self.targetdb == 'mysql':
            self.dsn = CONF["DSN"]
        ...
        try:
            while True:
                ...
                while True:
                    ...
                    elapsed = self._gen_sqlsmith_queries(5, 20)
                    if elapsed < 17:
                        self.log.debug(
                ...
                try:
                    ...
                    self.log.debug(
                        "Error when extract valid query",
                        _print=True)
                    continue
                ...
                for query in tqdm(queries):
                    if query.startswith('insert') or query.startswith('INSERT'):
                        print('Find a insert query:')
                        print(query)
                    if query.startswith('update') or query.startswith('UPDATE'):
                        print('Find a update query:')
                        print(query)
                    # 3.4) query complexity check
                    if self.targetdb == 'postgres':
                        query = "%s\n%s" % (CONF["PREFIX"], query)
                    elif self.targetdb == 'mysql':
                        query = "%s;\n%s\n%s\n%s;" % ("SET profiling=1", CONF["PREFIX"], query, "SHOW profiles")
                    elif self.targetdb == 'sqlite':
                        query = query + "\n"
                        if randint(0, 100) > 80:
                    ...
                    if result:
                        ...
                        print(div, elapsed_list[0], elapsed_list[1], div_val, elapsed_list_val[0], elapsed_list_val[1])
                        ...
                        self.log.debug(
                            ...
                            _print=True)
                        self.log.debug("QUERY", _print=True)
                        self.log.debug("====START====", _print=True)
                        self.log.debug(current_query, _print=True)
                        self.log.debug("====END====", _print=True)
                        self.log.debug(
                            " ".join(map(str, querying_time)), _print=True)
                        self.log.debug(
                        ...
                        result_path = os.path.join(
                            self.result_dir,
                            "%d" % (self.count_regression - 1))
                        with open(result_path + ".txt", 'w') as f:
                            f.write("(%f, %f, %f, %f, %f, %f)\n" %(div, elapsed_list[0], elapsed_list[1], div_val, elapsed_list_val[0], elapsed_list_val[1]))
                            f.write(
                                "[!] Found interesting case in %s\n" % current_test
                            )
                            f.write("QUERY\n")
                            f.write("====START====\n")
                            ...
                            f.write("====END====\n")
                            f.write(
                                " ".join(map(str, querying_time)) + "\n")
                            f.write(
                                "[!] %f times longer execution time\n" % div
                            )
                        with open(result_path + ".sql", 'w') as f:
                            f.write(current_query)
                        reduced_path = stored_pn + "_reduced"
                        if os.path.exists(reduced_path):
                            shutil.copy(reduced_path, self.result_dir)
                ...
                if self.targetdb == "mysql":
                    crashed = check_daemon_for_mysql()
                    if crashed:
                        self.log.debug("Daemon Break!", _print=True)
                        with open(
                            "%s/crash%d" %
    ...
    def dump_db(self, dbname):
        ...
        if self.targetdb == 'postgres':
            ...
            # mysqldump -S /tmp/mysql3306.sock -P 3306 -u root test_bd > /tmp/export.pgsql
            cmd = "mysqldump -S /tmp/mysql%d.sock -P %d -u root %s > %s" \
                % (LATEST_VERSION_PORT, LATEST_VERSION_PORT, dbname, pn)
    ...
    def import_db(self, pn, dbname, toAll=False):
        ...
        if self.targetdb == 'postgres':
            ...
            """
            e.g., mysql -S /tmp/mysql3306.sock -P 3306 -u root -D test_bd -e "SOURCE /tmp/export.pgsql"
            """
            cmd = "mysql -S /tmp/mysql{PORT}.sock \
                -P {PORT} -u root \
                -D %s -e \"SOURCE %s\"" \
                % (dbname, pn)
    ...
    def run_queries(self, dbname, query, no_stdout, tout=None):
        ...
        if self.targetdb == 'postgres':
            ...
            """
            e.g., mysql -S /tmp/mysql{PORT}.sock -P {PORT} -U root
            -D test_bd -e "select * from
            table1000_key_pk_parts_2_varchar_1024_not_null"
            """
            ...
            cmd = "%s \"SOURCE %s\"" % (CONF["RUN_OLD"], CURR_QUERY)
            ...
            elapsed_list=[]
            for port in SERVER_PORTS:
                current_stdout = stdout[port]
                elapsed_time = retrieve_actual_time_for_mysql(current_stdout)
                elapsed[port] = elapsed_time
    ...
    def drop_db(self, dbname):
        ...
        if self.targetdb == 'postgres':
            ...
            """
            e.g., mysql -S /tmp/mysql{PORT}.sock -P {PORT} -u root -e "drop database test_bd;"
            """
            cmd = "mysql -S /tmp/mysql{PORT}.sock \
                -P {PORT} -u root \
                -e 'drop database %s;'" % (dbname)
    ...
    def create_db(self, dbname):
        ...
        if self.targetdb == 'postgres':
            ...
            """
            e.g., mysql -S /tmp/mysql{PORT}.sock -P {PORT} -u root -e "create database test_bd"
            """
            cmd = "mysql -S /tmp/mysql{PORT}.sock \
                -P {PORT} -u root \
                -e 'create database %s;'" % (dbname)
    ...
    def _gen_sqlsmith_queries(self, query_num, timeout):
        ...
        if self.targetdb == 'postgres' or self.targetdb == 'cockroach':
            ...
            cmd = "timeout %ds %s --verbose --exclude-catalog \
            --dump-all-queries --seed=%d --max-queries=%d \
            --mysql=\"host=localhost port={PORT} socket=/tmp/mysql{PORT}.sock db=%s user=root password=\" 1> %s  2> %s" \
            % (timeout, SQLSMITH, randint(1, 1000000),
                query_num, TEST_DB, TMP_QUERY_PN, TMP_ERR_PN)
