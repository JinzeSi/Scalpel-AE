...
class Minimizer(object):
    ...
    def run_query_latest(self, query):
        ...
        elif self.dbname == 'mysql':
            elapsed, err, out = self.run_query(
                query, LATEST_VERSION_PORT, timeout=str(2))
            elapsed_new = retrieve_actual_time_for_mysql(out)
    ...
    def run_query(self, query, port, timeout=None):
        ...
        if self.dbname == "postgres":
            ...
            init_option = ""
            if timeout is not None:
                timeout_ms = max(1, int(float(timeout) * 1000) - 100)
                init_option = \
                    "--init-command='SET SESSION max_execution_time=%d'" \
                    % timeout_ms
            cmd = "mysql %s -S /tmp/mysql{PORT}.sock \
                -P {PORT} -u root \
                -D %s -e \"SOURCE %s\"" \
                    % (init_option, TEST_DB, TMP_QUERY_PN)
    ...
    def run_original_for_mysql(self):
        query = "analyze;" + self.org_query
        elapsed, err, out = self.run_query(
            query, LATEST_VERSION_PORT, timeout="7")
        elapsed2, err2, out2 = self.run_query(
        ...
        actual_latest = retrieve_actual_time_for_mysql(out)  # latest
        actual_old = retrieve_actual_time_for_mysql(out2)  # old
...
if __name__ == "__main__":
    ...
    if TARGET_DB == 'postgres':
        ...
        DSN = "\"dbi:mysql:host=127.0.0.1;port={PORT}\
         ;user=root;password=;database=%s\"" % (TEST_DB)
