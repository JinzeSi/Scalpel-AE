...
Function * Matcher::matchFunction(sp_iterator & I, Scope &scope, bool & multiple)
{
  ...
  while (I != E) {
    ...
    if (I->lastline == 0) {
      ...
      bool next_is_same_file = (I + 1 != E) &&
                               (I->filename == (I + 1)->filename) &&
                               (I->directory == (I + 1)->directory);
      if (!next_is_same_file) {
        // 情况 A: 当前函数是该文件的最后一个函数（因为下一个函数属于不同的文件，或者没有下一个函数）
        // 我们无法根据下一个函数来推断当前函数的结束行。
        // 既然无法精确知道，安全的做法是假设该函数一直延伸到当前文件的末尾。
        // 设置为 INT_MAX (0x7fffffff) 可以确保文件后续所有的补丁块都能被归纳进来进行匹配尝试。
        I->lastline = 0x7fffffff; 
      } else {
