...
# ================= Configuration =================
PROJECT_ROOT="/perfscope/mysql-server"
...
CMAKE_BIN_PATH="/perfscope/cmake/bin/cmake"

# 结果输出文件
RESULT_LOG="mysql.res"
...
echo "  -> Patching CMake files to disable MINIMAL_RELWITHDEBINFO..."

# 1. 查找包含该逻辑的文件（通常是 CMakeLists.txt 或 cmake/compiler_options.cmake）
# 2. 使用 sed 将 IF(MINIMAL_RELWITHDEBINFO) 到 ENDIF() 之间的每一行开头加上 #
grep -rl "MINIMAL_RELWITHDEBINFO" . | xargs -r sed -i '/IF(MINIMAL_RELWITHDEBINFO)/,/ENDIF()/ s/^/#/'

echo "  -> Patch applied (if found)."
...
# 生成 diff 文件
DIFF_FILE="patch/mysql.diff"
# 使用标准 unified diff 格式
git show "$COMMIT_ID" > "$DIFF_FILE"
...
echo "  -> Running CMake..."
"$CMAKE_BIN_PATH" -DCMAKE_CXX_FLAGS_RELWITHDEBINFO="-O0 -g -DNDEBUG" \
      -DCMAKE_BUILD_TYPE=RelWithDebInfo \
      -DWITH_BOOST=$PROJECT_ROOT/boost_1_77_0 \
      .. > cmake_log.txt 2>&1

# 编译 mysqld
echo "  -> Running Make mysqld (this may take a while)..."
make mysqld -j$(nproc) > make_log.txt 2>&1

# 提取 Bitcode
echo "  -> Extracting Bitcode..."
extract-bc bin/mysqld

BC_FILE="$PROJECT_ROOT/build/bin/mysqld.bc"
if [ ! -f "$BC_FILE" ]; then
    echo "Error: mysqld.bc not found after compilation."
    exit 1
fi
...
PROFILE_FILE="$PROJECT_ROOT/mysql.profile"
