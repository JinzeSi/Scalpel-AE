...
void static_profile(Module * module, CostModel * model)
{
  ...
  vector<FuncCost> func_cost(size);
  vector<FuncHot> func_hot(size);
  unsigned i = 0;
  ...
  fprintf(fout, "====\n");
  std::sort(func_cost.begin(), func_cost.end(), [](const FuncCost& a, const FuncCost& b) -> bool { return a.cost > b.cost; });
  size_t k = (!printall && topk_cost > 0 && topk_cost < size) ? topk_cost : size;
  for (i = 0; i < k ; i++) {
  ...
  fprintf(fout, "====\n");
  std::sort(func_hot.begin(), func_hot.end(), [](const FuncHot& a, const FuncHot& b) -> bool { return a.hotness > b.hotness; });
  k = (!printall && topk_hot > 0 && topk_hot < size) ? topk_hot: size;
  for (i = 0; i < k; i++) {
