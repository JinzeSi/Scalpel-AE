...
static void emitPrivatesInit(CodeGenFunction &CGF,
  ...
  LValue SrcBase;
  bool IsTargetTask =
      isOpenMPDataLayoutManagementDirective(D.getDirectiveKind()) ||
...
emitOffloadingArrays(CodeGenFunction &CGF,
                     ...
                     CGOpenMPRuntime::DataLayoutInfo &Info) {
...
static void emitOffloadingArraysArgument(
    ...
    llvm::Value *&MapTypesArrayArg, CGOpenMPRuntime::DataLayoutInfo &Info) {
  CodeGenModule &CGM = CGF.CGM;
  if (Info.NumberOfPtrs) {
...
void CGOpenMPRuntime::emitTargetCall(CodeGenFunction &CGF,
  ...
  CodeGenFunction::OMPDataLayoutInfo InputInfo;
...
void CGOpenMPRuntime::emitDataLayoutCalls(
    CodeGenFunction &CGF, const OMPExecutableDirective &D, const Expr *IfCond,
    const Expr *Device, const RegionCodeGenTy &CodeGen, DataLayoutInfo &Info) {
  if (!CGF.HaveInsertPoint())
    return;
...
void CGOpenMPRuntime::emitDataLayoutStandAloneCall(
    CodeGenFunction &CGF, const OMPExecutableDirective &D, const Expr *IfCond,
    const Expr *Device) {
  if (!CGF.HaveInsertPoint())
    return;
...
void CGOpenMPSIMDRuntime::emitDataLayoutCalls(
    CodeGenFunction &CGF, const OMPExecutableDirective &D, const Expr *IfCond,
    const Expr *Device, const RegionCodeGenTy &CodeGen, DataLayoutInfo &Info) {
  llvm_unreachable("Not supported in SIMD-only mode");
}

void CGOpenMPSIMDRuntime::emitDataLayoutStandAloneCall(
    CodeGenFunction &CGF, const OMPExecutableDirective &D, const Expr *IfCond,
    const Expr *Device) {
  llvm_unreachable("Not supported in SIMD-only mode");
}
