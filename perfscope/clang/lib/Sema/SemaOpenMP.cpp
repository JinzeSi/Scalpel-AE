...
void Sema::tryCaptureOpenMPLambdas(ValueDecl *V) {
  ...
  if (!CurContext->isDependentContext() &&
      (isOpenMPTargetExecutionDirective(DSAStack->getCurrentDirective()) ||
       isOpenMPDataLayoutManagementDirective(
...
static bool checkNestingOfRegions(Sema &SemaRef, const DSAStackTy *Stack,
  ...
  if (Stack->getCurScope()) {
    ...
    if (!NestingProhibited &&
        ...
        !isOpenMPDataLayoutManagementDirective(CurrentRegion) &&
...
StmtResult Sema::ActOnOpenMPExecutableDirective(
  ...
  if (AStmt && !CurContext->isDependentContext()) {
    ...
    DSAChecker.Visit(S);
    if (!isOpenMPDataLayoutManagementDirective(Kind) &&
  ...
  case OMPD_cancel:
    ...
    break;
  case OMPD_target_data:
    Res = ActOnOpenMPDataLayoutDirective(ClausesWithImplicit, AStmt, StartLoc,
                                         EndLoc);
...
StmtResult Sema::ActOnOpenMPDataLayoutDirective(ArrayRef<OMPClause *> Clauses,
                                                Stmt *AStmt,
                                                SourceLocation StartLoc,
                                                SourceLocation EndLoc) {
  if (!AStmt)
  ...
  return OMPDataLayoutDirective::Create(Context, StartLoc, EndLoc, Clauses,
                                        AStmt);
}
