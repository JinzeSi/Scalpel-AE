...
# ================= Configuration =================
PROJECT_ROOT="/perfscope/mariadb-server"
...
CMAKE_BIN_PATH="/perfscope/cmake-3.15.0/bin/cmake"

# 结果输出文件
RESULT_LOG="mariadb.res"
...
# 编译 mariadbd
echo "  -> Running Make Mariadbd (this may take a while)..."
make mariadbd -j$(nproc) > make_log.txt 2>&1

# 提取 Bitcode
echo "  -> Extracting Bitcode..."
extract-bc sql/mariadbd

BC_FILE="$PROJECT_ROOT/build/sql/mariadbd.bc"
if [ ! -f "$BC_FILE" ]; then
    echo "Error: mariadbd.bc not found after compilation."
    exit 1
fi
...
PROFILE_FILE="$PROJECT_ROOT/mariadb.profile"
