...
void GraphBuilder::visitLandingPadInst(LandingPadInst &I) {
  // LandingPad 指令返回一个结构体 { i8*, i32 }，第一个元素是异常指针。
  // 我们创建一个新的 DSNode 来代表这个异常对象。
  DSNode *Node = createNode();
  
  // 标记为 Incomplete，因为我们不知道它具体来自哪里（外部抛出的）
  Node->setIncompleteMarker();
  Node->setReadMarker();
  // 标记为 Modified，因为异常处理流程可能会写入它
  Node->setModifiedMarker();
  // 折叠节点，因为异常对象的结构通常是不透明或混乱的
  Node->foldNodeCompletely();

  // 将 LandingPad 指令的结果（作为值）指向这个节点
  // 注意：虽然 LandingPad 返回的是聚合体，但在 LLVM IR 中它本身作为一个 Value
  // 后续通常会用 ExtractValue 来取指针。
  // 为了保守起见，我们将整个指令的结果关联到这个 Unknown Node。
  setDestTo(I, Node);
}
...
void GraphBuilder::visitVAStartInst(VAStartInst &I) {
  CallSite CS(&I);
...
bool GraphBuilder::visitIntrinsic(CallSite CS, Function *F) {
  ...
  case Intrinsic::vastart: {
    visitVAStartInst(*cast<VAStartInst>(CS.getInstruction()));
    return true;
  }
...
bool LocalDataStructures::runOnModule(Module &M) {
  init(const_cast<llvm::DataLayout*>(&M.getDataLayout()));
