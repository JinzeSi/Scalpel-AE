...
echo "[3/7] Compiling Redis with WLLVM (Debug Info Enabled)..."

OVERRIDE_BIN="$PROJECT_ROOT/llvm-tools-override"
...
# 编译 redis
echo "  -> Running Make redis (this may take a while)..."
CC=wllvm make noopt OPTIMIZATION="-O0" -j$(nproc) > make_log.txt 2>&1

# 提取 Bitcode
echo "  -> Extracting Bitcode..."
extract-bc src/redis-server

BC_FILE="$PROJECT_ROOT/src/redis-server.bc"
if [ ! -f "$BC_FILE" ]; then
    echo "Error: redis-server.bc not found after compilation."
    exit 1
fi
...
PROFILE_FILE="$PROJECT_ROOT/redis.profile"
