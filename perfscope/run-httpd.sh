...
echo "[3/7] Compiling Httpd with WLLVM (Debug Info Enabled)..."

# 准备 build-bc 目录
if [ -d "build-bc" ]; then
    echo "  -> Cleaning existing build-bc directory..."
    rm -rf build-bc/*
else
    mkdir -p build-bc
fi

OVERRIDE_BIN="$PROJECT_ROOT/build-bc/llvm-tools-override"
...
CC=wllvm ../configure --prefix=$(pwd)/app --enable-mods-static=reallyall --enable-load-all-modules --with-crypto CFLAGS="-g -O0" > config_log.txt 2>&1

# 编译 httpd
echo "  -> Running Make httpd (this may take a while)..."
CC=wllvm make -j$(nproc) > make_log.txt 2>&1
CC=wllvm make install

# 提取 Bitcode
echo "  -> Extracting Bitcode..."
extract-bc app/bin/httpd

BC_FILE="$PROJECT_ROOT/build-bc/app/bin/httpd.bc"
if [ ! -f "$BC_FILE" ]; then
    echo "Error: httpd.bc not found after compilation."
    exit 1
fi
...
PROFILE_FILE="$PROJECT_ROOT/httpd.profile"
